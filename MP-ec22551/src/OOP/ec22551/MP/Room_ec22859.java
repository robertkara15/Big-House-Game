...
class Room_ec22859 extends Room{
    ...
    private char[] choices = {'1', '2'}; // Decision making array for the visitor
    ...
    Boolean findTorch(Visitor v){
        ...
        char choicemade = v.getChoice("Do you want to (1)risk looking around for a lighter or (2)leave?", choices);
        if(choicemade == '1'){
    ...
    Boolean fightOrc(Visitor v){
        ...
        char choicemade = v.getChoice("Do you want to (1)risk fighting for a reward or (2)leave?", choices);
        if(choicemade == '1'){
            int dice = ThreadLocalRandom.current().nextInt(0, 2);
            if(dice == 1 | dice == 2){
                hasWon = true;
